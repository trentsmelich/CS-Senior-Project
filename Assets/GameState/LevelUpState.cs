...
public class LevelUpState : GameState
{
    ...
    public List<string> playerStats = new List<string> { "Health", "Speed", "Strength", "Attack Speed", "Profit Multiplier", "Experience Multiplier" };
    public override void EnterState(GameStateController Game)
    {
        ...
        offer1.GetComponentInChildren<Text>().text = $"{selectedStats[0]} +{statIncreases[0]}";
        offer2.GetComponentInChildren<Text>().text = $"{selectedStats[1]} +{statIncreases[1]}";
        offer3.GetComponentInChildren<Text>().text = $"{selectedStats[2]} +{statIncreases[2]}";
